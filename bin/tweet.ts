    ...
    if (argv.flags.interactive) {
      ...
      if (argv.flags.thread) {
        await generateThread(
          ...
          twitterClient,
          argv.flags.prompt,
          argv.flags.maxTweets,
          argv.flags.dryRun
        );
      } else {
        await generateSingleTweet(
          engine,
          twitterClient,
          argv.flags.prompt,
          argv.flags.dryRun
...
async function generateSingleTweet(
  ...
  if (!dryRun && client) {
    console.log(chalk.yellow('\n📤 Posting to Twitter...'));
    const posted = await (client as any).tweet(tweet);
...
async function reactToTopic(
  ...
  const tweet = await engine.reactToTrend(topic, sentiment as any);
