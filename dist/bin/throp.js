...
async function main() {
    ...
    try {
        ...
        if (argv.flags.answerEngine) {
            config.bot.answerEngine = argv.flags.answerEngine;
        }
        if (argv.flags.maxMentions) {
            config.bot.maxMentionsPerBatch = argv.flags.maxMentions;
        }
 ...
 */
async function runBot(client, config, flags) {
    logger.header('Processing Mentions');
    try {
        // Fetch mentions
        const mentions = await client.getMentions(flags.sinceMentionId, config.bot.maxMentionsPerBatch);
