...
export class GeckoTerminalTool {
     ...
     */
    async getTokenPrice(token, network = 'solana') {
    ...
    async searchPools(query, network) {
        try {
            ...
            if (this.looksLikeAddress(query)) {
                ...
                const tokenResponse = await ky.get(tokenUrl).json();
     ...
     */
    async getOHLCV(poolAddress, timeframe = '4h') {
        try {
     ...
     * Analyze chart data
     */
    async analyzeChart(token, network = 'solana') {
        try {
     ...
     */
    performTechnicalAnalysis(ohlcv) {
