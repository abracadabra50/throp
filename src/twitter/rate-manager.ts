...
export class RateLimitManager {
  ...
  private tweets: number[] = []; // Timestamps of tweets sent
  
  // Limits (for basic plan)
  private readonly MAX_API_CALLS_PER_15_MIN = 15;
  private readonly MAX_TWEETS_PER_DAY = 50;
  private readonly MAX_TWEETS_PER_HOUR = 3; // Be conservative
   ...
   */
  async canMakeApiCall(): Promise<boolean> {
    this.cleanOldEntries();
    
    const recentCalls = this.apiCalls.filter(
      time => time > Date.now() - (15 * 60 * 1000) // Last 15 minutes
    );
   ...
   */
  async canSendTweet(): Promise<boolean> {
    this.cleanOldEntries();
    
    // Check daily limit
    const todaysTweets = this.tweets.filter(
    ...
    // Check hourly limit
    const lastHourTweets = this.tweets.filter(
  ...
  recordApiCall(): void {
    ...
    logger.debug('API call recorded', {
      totalIn15Min: this.apiCalls.filter(
        t => t > Date.now() - (15 * 60 * 1000)
      ).length,
    });
  }
  ...
  getStats() {
    ...
    return {
      apiCalls: {
        last15Min: this.apiCalls.filter(
          t => t > Date.now() - (15 * 60 * 1000)
        ).length,
        limit: this.MAX_API_CALLS_PER_15_MIN,
      },
      tweets: {
        today: this.tweets.filter(
          t => t > Date.now() - (24 * 60 * 60 * 1000)
        ).length,
        lastHour: this.tweets.filter(
