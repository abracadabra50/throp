 ...
 */
function validateAnswerEngine(config: Config): void {
 ...
 */
function validateTwitterCredentials(config: Config): void {
 ...
 */
export function getRateLimits(plan: Config['twitter']['apiPlan']) {
  const limits = {
    basic: {
      tweetsPerMonth: 1500,
      tweetsPerDay: 50,
 ...
 */
export function calculateRequestDelay(plan: Config['twitter']['apiPlan']): number {
 ...
 */
export function canSendMoreTweets(tweetCount: number, plan: Config['twitter']['apiPlan']): boolean {
  const limits = getRateLimits(plan);
