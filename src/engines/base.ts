 ...
 */
export abstract class BaseAnswerEngine {
  protected name: string;
  protected maxTokens: number;
  protected temperature: number;

  constructor(name: string, maxTokens = 1000, temperature = 0.7) {
    this.name = name;
   ...
   */
  protected formatForTwitter(response: string, maxLength = 280): string[] {
    ...
    if (tweets.length > 1) {
      return tweets.map((tweet, index) => 
        `[${index + 1}/${tweets.length}] ${tweet}`.substring(0, maxLength)
      );
    }
   ...
   */
  protected shouldThread(response: string, maxLength = 280): boolean {
