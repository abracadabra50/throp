...
export class ApiServer {
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/api/trending-prompts', async (req: Request, res: Response) => {
    ...
    this.app.post('/api/chat', async (req: Request, res: Response) => {
      try {
        ...
        // Return full response if threaded, not just first part
        const fullResponse = response.threadParts ? 
          response.threadParts.join(' ') : 
          response.text;
    ...
    this.app.get('/api/mentions', async (req: Request, res: Response) => {
      try {
    ...
    this.app.post('/api/mentions/trigger', async (_req: Request, res: Response) => {
      try {
        if (!this.twitterClient) {
          return res.status(503).json({
            success: false,
    ...
    this.app.post('/api/mentions/:id/process', async (req: Request, res: Response) => {
      try {
        if (!this.twitterClient) {
          return res.status(503).json({
            success: false,
    ...
    this.app.get('/api/cache/interactions', async (req: Request, res: Response) => {
      try {
  ...
  private setupWebSocket(): void {
    this.io.on('connection', (socket) => {
      ...
      socket.on('chat', async (data: ChatRequest) => {
        try {
          const context: AnswerContext = {
   ...
   */
  private async saveProcessedMentions(): Promise<void> {
