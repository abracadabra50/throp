...
/**
 * Request types for tweet endpoints
 */
interface ProactiveTweetRequest {
  prompt: string;
  context?: any;
  dryRun?: boolean;
  thread?: boolean;
...
interface TrendReactionRequest {
  topic: string;
  sentiment?: 'bullish' | 'bearish' | 'neutral';
  dryRun?: boolean;
}

interface ScheduledTweetRequest {
  prompt: string;
 ...
 */
export function createTweetRoutes(): Router {
  const router = Router();
  ...
  router.post('/prompt', adminAuth, adminRateLimit(), async (req, res) => {
    try {
      const { prompt, dryRun = true, thread = false, maxTweets = 5 } = req.body as ProactiveTweetRequest;
      ...
      if (thread) {
        ...
        if (!dryRun && twitterClient) {
          // Post the tweet
          const posted = await (twitterClient as any).tweet(tweet);
          result = {
            type: 'single',
            tweet,
            tweetId: posted.id,
  ...
  router.post('/react', adminAuth, adminRateLimit(), async (req, res) => {
    try {
      const { topic, sentiment = 'neutral', dryRun = true } = req.body as TrendReactionRequest;
  ...
  router.post('/schedule', adminAuth, adminRateLimit(), async (req, res) => {
    try {
      const { prompt, scheduleTime, recurring, dryRun = true } = req.body as ScheduledTweetRequest;
...
// Extension for TwitterClient to add tweet method
...
TwitterClient.prototype.tweet = async function(text: string): Promise<any> {
  if ((this as any).config.bot.dryRun) {
    logger.info('[DRY RUN] Would tweet', { text });
    return { id: 'dry-run-id', text };
  }
  
  try {
    const response = await (this as any).executeWithRetry(async () => {
      return await (this as any).v2Client.tweet(text);
    });
    ...
    throw (this as any).wrapError(error);
  }
};
